...
final class MultiplexedUtils {
    ...
    static {
        String currentXSocketVersion = ConnectionUtils.getImplementationVersion();
        String requiredXSocketVersion = getXSocketImplementationVersion();
        if (!ConnectionUtils.matchVersion(currentXSocketVersion, requiredXSocketVersion)) {
            LOG.warning("xSocket-multiplexed " + getImplementationVersion() + " requires xSocket " + requiredXSocketVersion + ". Current xSocket version is " + currentXSocketVersion);
        }
    }
