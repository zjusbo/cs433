...
public final class MultiplexedConnection implements IMultiplexedConnection {
	...
	private final class NonBlockingPipeline extends AbstractNonBlockingStream implements INonBlockingPipeline {
		...
		// handler
		private final AtomicReference<PipelineHandlerAdapter> handler = new AtomicReference<PipelineHandlerAdapter>(null);
		...
		private void onOpen() {
			try {
				handler.get().onConnect(this, taskQueue);
		...
		private void onClose() {
			...
			try {
				handler.get().onDisconnect(this, taskQueue);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) 
		...
		public void onData(ByteBuffer[] data) {
			...
			try {
				...
				handler.get().onData(this, taskQueue);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
		...
		private void onConnectionTimeout() {
			if (!connectionTimeoutOccured) {
				...
				try {
					handler.get().onConnectionTimeout(this, taskQueue);
				} catch (IOException ioe) {
					if (LOG.isLoggable(Level.FINE)) 
		...
		private void onIdleTimeout() {
			if (!idleTimeoutOccured) {
				...
				try {
					handler.get().onIdleTimeout(this, taskQueue);
				} catch (IOException ioe) {
					if (LOG.isLoggable(Level.FINE)) 
		...
		public IHandler getHandler() {
			PipelineHandlerAdapter hdlAdapter = handler.get();
			if (hdlAdapter == null) {
				return null;
			} else {
