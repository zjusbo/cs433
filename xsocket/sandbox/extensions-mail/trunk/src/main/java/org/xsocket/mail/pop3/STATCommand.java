 ...
 */
@CommandName("STAT")
final class STATCommand extends AbstractPOP3Command {
	...
	public void execute(ICommandContext ctx, INonBlockingConnection connection, String[] arguments) throws IOException, POP3ProtocolException {
		...
		for (Integer msgNumber : msgNumbers) {
			size += ctx.getMaildrop().getMessageSize(ctx.getMessageUid(msgNumber));
		}			
		sendPositiveResponse(connection, msgNumbers.length + " " + size);
	}
}
