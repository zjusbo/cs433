 ...
 */
@CommandName("RCPT TO:")
final class SmtpRCPTTOCommand extends SmtpCommand {
	
	public SmtpRCPTTOCommand() {
	...
	public void execute(INonBlockingConnection connection, ISmtpSession session, String argument) throws SmtpProtocolException, IOException {
		...
		// check forward-path
		checkTrue(argument.length() > 0, 504, " missing parameter");
		
		try {
			session.getSmtpForwardPaths().add(argument);
			session.getMessageSinkService().isForwardPathAccepted(session);
			sendResponse(connection, "250 " + argument + " Receiver OK");
		} catch (SmtpRejectException re) {
			session.getSmtpForwardPaths().remove(argument);
			throw re;
		}
	}
