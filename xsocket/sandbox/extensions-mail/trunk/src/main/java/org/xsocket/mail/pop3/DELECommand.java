 ...
 */
@CommandName("DELE") 
final class DELECommand extends AbstractPOP3Command {
	...
	public void execute(ICommandContext ctx, INonBlockingConnection connection, String[] arguments) throws IOException, POP3ProtocolException {
		...
		Integer number = parseIntArgument(arguments[0]);
		boolean wasNotMarked = ctx.addMessageNumberToDelete(number);
		if (wasNotMarked) {
			sendPositiveResponse(connection, "message " + number + " deleted");
		} else {
			sendNegativeResponse(connection, "message " + number + " is already deleted");
		} 		
	}
}
