 ...
 */
@CommandName("LIST") 
final class LISTCommand extends AbstractPOP3Command {
	...
	public void execute(ICommandContext ctx, INonBlockingConnection connection, String[] arguments) throws IOException, POP3ProtocolException {
		...
		if (arguments.length == 1) {
			...
			if (ctx.isMessageNumberMarkedAsDeleted(msgNumber)) {
				...
				if (uid != null) {
					Integer size = ctx.getMaildrop().getMessageSize(uid);
					sendPositiveResponse(connection, msgNumber + " " + size);
				} else {
			...
			if (msgNumbers.length > 0) {
				...
				for (Integer msgNumber : msgNumbers) {
					if (!ctx.isMessageNumberMarkedAsDeleted(msgNumber)) {
						sendMultilineResponse(connection, msgNumber + " " + ctx.getMaildrop().getMessageSize(ctx.getMessageUid(msgNumber)));
					}
				}
