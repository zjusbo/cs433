// $Id: IntrospectionBasedDynamicBean.java 1037 2007-03-20 06:45:01Z grro $
/*
...
final class IntrospectionBasedDynamicBean implements DynamicMBean {
			...
			if ((name.length() > 3) && name.startsWith("get")) {
				if (method.getParameterTypes().length == 0) {
					...
					if(isAcceptedPropertyType(propertyType)) {
						Info info = registerInfo(name.substring(3, name.length()));
	...
	private Info registerInfo(String name) {
