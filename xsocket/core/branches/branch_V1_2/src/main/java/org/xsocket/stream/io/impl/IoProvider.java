...
public final class IoProvider implements IClientIoProvider, IServerIoProvider {
    ...
    static {
    	...
    	// Memory properties
    	try {
    		useDirectReadBufferClient = Boolean.valueOf(System.getProperty(IoProvider.USE_DIRECT_READ_BUFFER_CLIENT_KEY, Boolean.toString(DEFAULT_USE_DIRECT_BUFFER)));
    	} catch (Exception e) {
   		...
   		try {
   			useDirectReadBufferServer = Boolean.valueOf(System.getProperty(IoProvider.USE_DIRECT_READ_BUFFER_SERVER_KEY, Boolean.toString(DEFAULT_USE_DIRECT_BUFFER)));
   		} catch (Exception e) {
		...
		if (LOG.isLoggable(Level.FINE)) {
			LOG.fine(IoProvider.class.getName() + " initialized (useDirectReadBufferClient=" + useDirectReadBufferClient 
				   + " readBufferPreallocationsizeClient=" + DataConverter.toFormatedBytesSize(readBufferPreallocationsizeClient) 
				   + " useDirectReadBufferServer=" + useDirectReadBufferServer 
				   + " readBufferPreallocationsizeServer=" + DataConverter.toFormatedBytesSize(readBufferPreallocationsizeServer)
				   + ")");
		}
     }
