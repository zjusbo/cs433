// $Id: MulticastEndpoint.java 1770 2007-09-26 10:45:21Z grro $
/*
...
public final class MulticastEndpoint extends AbstractEndpoint implements IConnectedEndpoint {
	...
	public MulticastEndpoint(InetAddress address, final int port) throws IOException {
		this(address, port, new DatagramSocketConfiguration(), 0, null);
	}
	...
	public MulticastEndpoint(String address, final int port, int receiveSize, IDatagramHandler datagramHandler) throws IOException {
		this(InetAddress.getByName(address), port, new DatagramSocketConfiguration(), receiveSize, datagramHandler);
	}
	...
	public MulticastEndpoint(InetAddress address, final int port, int receiveSize, IDatagramHandler datagramHandler, Executor workerPool) throws IOException {
		this(address, port, new DatagramSocketConfiguration(), receiveSize, datagramHandler, workerPool);
	}
	...
	public MulticastEndpoint(InetAddress address, int port, DatagramSocketConfiguration socketConfiguration, int receiveSize, IDatagramHandler datagramHandler) throws IOException {
		this(address, port, socketConfiguration, receiveSize, datagramHandler, getGlobalWorkerPool());
	}
    ...
    /**
  	 * @deprecated
     */
	public MulticastEndpoint(InetAddress address, int port, DatagramSocketConfiguration socketConfiguration, int receiveSize, IDatagramHandler datagramHandler, Executor workerPool) throws IOException {
		this(address, port, socketConfiguration.toOptions(), receiveSize, datagramHandler, workerPool);
	}
	...
	public MulticastEndpoint(InetAddress address, int port, Map<String, Object> options, int receiveSize, IDatagramHandler datagramHandler, Executor workerPool) throws IOException {
		...
		for (Entry<String, Object> entrySet : options.entrySet()) {
			setOption(entrySet.getKey(), entrySet.getValue());
		}
