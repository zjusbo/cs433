...
class HandlerAdapter  {
                    ...
                    public void run() {
                        try {
                            ...
                            if (LOG.isLoggable(Level.FINE)) {
                                LOG.fine("[" + connection.getId() + "] error occured while performing onConnect multithreaded " + getHandler() + " " + ioe.toString());
                            }
                        }
                    }
                };
    ...
    public boolean onData(INonBlockingConnection connection, SerializedTaskQueue taskQueue, boolean ignoreException, boolean isUnsynchronized) throws IOException, BufferUnderflowException, MaxReadSizeExceededException {
        if (handlerInfo.isDataHandler()) {
            ...
            if (LOG.isLoggable(Level.FINE)) {
                LOG.fine("[" + connection.getId() + "] assigned handler " + handler + " is not a data handler");
            }
        }
        return true;
    }   
        ...
        public void run() {
            try {
               performOnData(connection, taskQueue, ignoreException);
                ...
                if (LOG.isLoggable(Level.FINE)) {
                    LOG.fine("[" + connection.getId() + "] error occured while performing onData multithreaded " + getHandler() + " " + ioe.toString());
                }
            }
        }
	...
	private boolean performOnData(INonBlockingConnection connection, SerializedTaskQueue taskQueue, boolean ignoreException) throws IOException, BufferUnderflowException, MaxReadSizeExceededException {
        ...
        while ((connection.available() != 0) && !connection.isReceivingSuspended()) {
            ...
            try {
                if (LOG.isLoggable(Level.FINE)) {
                    LOG.fine("[" + connection.getId() + "] calling onData method of handler " + handler);
                }
            	...
            	if (!ignoreException) {
	                if (LOG.isLoggable(Level.FINE)) {
	                    LOG.fine("[" + connection.getId() + "] ERROR closing connection because an error has been occured by handling data by appHandler. " + handler + " Reason: " + DataConverter.toString(re));
	                }
	                closeSilence(connection);
            	}
                throw re;
            	...
            	if (!ignoreException) {
	                if (LOG.isLoggable(Level.FINE)) {
	                    LOG.fine("[" + connection.getId() + "] closing connection because an error has been occured by handling data by appHandler. " + handler + " Reason: " + DataConverter.toString(ioe));
	                }
	                closeSilence(connection);
            	}
                throw ioe;
    ...
    private final class PerformOnDisconnectTask implements Runnable {
        ...
        public void run() {
            try {
                ...
                if (LOG.isLoggable(Level.FINE)) {
                    LOG.fine("[" + connection.getId() + "] error occured while performing onDisconnect multithreaded " + getHandler() + " " + ioe.toString());
                }
            }
        }
                ...
                Runnable task = new Runnable() {
                    public void run() {
                        try {
                            ...
                            if (LOG.isLoggable(Level.FINE)) {
                                LOG.fine("[" + connection.getId() + "] error occured while performing onDisconnect multithreaded " + getHandler() + " " + e.toString());
                            }
                        }
                    }
                };
                ...
                Runnable task = new Runnable() {
                    public void run() {
                        try {
                            ...
                            if (LOG.isLoggable(Level.FINE)) {
                                LOG.fine("[" + connection.getId() + "] error occured while performing onIdleTimeout multithreaded " + getHandler() + " " + ioe.toString());
                            }
                        }
                    }
                };
                ...
                Runnable task = new Runnable() {
                    public void run() {
                        try {
                            ...
                            if (LOG.isLoggable(Level.FINE)) {
                                LOG.fine("[" + connection.getId() + "] error occured while performing onConnectionTimeout multithreaded " + getHandler() + " " + ioe.toString());
                            }
                        }
                    }
                };
