// $Id: NonBlockingConnectionClientTest.java 1281 2007-05-29 19:48:07Z grro $
/*
...
public final class NonBlockingConnectionClientTest {
	...
	@Test public void testDelayedWrite() throws Exception {
		...
		Assert.assertTrue(inTimeRange(elapsed, 0, 0, 500)); 
		...
		Assert.assertTrue(isEquals(tenByteRequest, response));
		Assert.assertTrue(inTimeRange(elapsed, 2500, 2000, 3000));  // 10 byte / 5 bytesPerSec -> 2,5 sec
		...
		Assert.assertTrue(isEquals(tenByteRequest, response));
		Assert.assertTrue(inTimeRange(elapsed, 3333, 2800, 4500)); // 10 byte / 3 bytesPerSec -> 3,3333  sec
