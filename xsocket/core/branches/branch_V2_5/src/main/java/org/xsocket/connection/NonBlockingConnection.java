...
public final class NonBlockingConnection extends AbstractNonBlockingStream implements INonBlockingConnection {
	...
	private void onPostData() {
		...
		try {
			handlerAdapter.get().onData(NonBlockingConnection.this, taskQueue, workerpool, false, false);
		} catch (IOException ioe) {
			if (LOG.isLoggable(Level.FINE)) {
	...
	private void onConnect() {
		try {
			handlerAdapter.get().onConnect(NonBlockingConnection.this, taskQueue, workerpool);
		} catch (IOException ioe) {
			if (LOG.isLoggable(Level.FINE)) {
	...
	private void onDisconnect() {
		...
		if (!disconnectOccured.getAndSet(true)) {
			...
			// call first onData
			try {
				handlerAdapter.get().onData(NonBlockingConnection.this, taskQueue, workerpool, true, false);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("[" + getId() + "] error occured by performing onData callback on " + handlerAdapter.get() + " " + ioe.toString());
				}
			...
			// then onDisconnect
			try {
				handlerAdapter.get().onDisconnect(NonBlockingConnection.this, taskQueue, workerpool, false);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
	...
	private void onConnectException(IOException ioe) {
        ...
        if (!connectExceptionOccured.getAndSet(true)) {
            try {
                handlerAdapter.get().onConnectException(NonBlockingConnection.this, taskQueue, workerpool, ioe);
            } catch (IOException e) {
                if (LOG.isLoggable(Level.FINE)) {
	...
	void onIdleTimeout() {
		if (!idleTimeoutOccured.getAndSet(true)) {
			try {
				handlerAdapter.get().onIdleTimeout(NonBlockingConnection.this, taskQueue, workerpool);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
	...
	private void onConnectionTimeout() {
		if (!connectionTimeoutOccured.getAndSet(true)) {
			try {
				handlerAdapter.get().onConnectionTimeout(NonBlockingConnection.this, taskQueue, workerpool);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
