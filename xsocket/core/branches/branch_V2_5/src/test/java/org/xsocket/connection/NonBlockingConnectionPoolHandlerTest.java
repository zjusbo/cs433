...
public final class NonBlockingConnectionPoolHandlerTest {
				...
				public void run() {
					...
					try {
						...
						for (int i = 0; i < 3; i++) {
							...
							do {
								QAUtil.sleep(50);
							} while (clientHdl.response.get() == null);


							if(!clientHdl.response.get().endsWith("test")) {
	...
	public void testUpdateHandler() throws Exception {
		...
		Assert.assertTrue("test".equals(ch1.response.get()));
		con.close();
	...
	private static final class NonThreadedClientHandler implements IConnectHandler, IDataHandler, IDisconnectHandler, IIdleTimeoutHandler, IConnectionTimeoutHandler {
		...
		private AtomicReference<String> response = new AtomicReference<String>(null);
		...
		public boolean onData(INonBlockingConnection connection) throws IOException, BufferUnderflowException, MaxReadSizeExceededException {
			threadname = Thread.currentThread().getName();
			response.set(connection.readStringByDelimiter(EchoHandler.DELIMITER));
