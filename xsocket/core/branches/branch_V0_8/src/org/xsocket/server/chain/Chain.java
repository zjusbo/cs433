			...
			while (node != null) {
				...
				if (handler != null) {
					...
					try {
						...
						// if is connection still open -> call successor
						...
						if (LOG.isLoggable(Level.FINE)) {
							LOG.fine("error occured within onConnectionOpening by " + handler + ". Reason: " + e.toString());
							return true;
						}
					}
			...
			while (node != null) {
				...
				if (handler != null) {
					...
					try {
						...
						if (LOG.isLoggable(Level.FINE)) {
							LOG.fine("error occured within onData by " + handler + ". Reason: " + e.toString());
							return true;
						}
					}
