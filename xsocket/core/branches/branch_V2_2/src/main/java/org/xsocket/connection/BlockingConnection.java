...
*    * @throws IOException If some other I/O error occurs
	...
	private BlockingConnection(InetSocketAddress remoteAddress, int connectTimeoutMillis, Map<String, Object> options, SSLContext sslContext, boolean sslOn) throws IOException {
		setUnderlyingConnection(new NonBlockingConnection(remoteAddress, true, connectTimeoutMillis, options, sslContext, sslOn, handler, new SingleThreadedWorkerPool()));
	}
...
//		delegee.setMaxWriteBufferThreshold(size);
	...
	private final class ReadNotificationHandler implements IDataHandler, IDisconnectHandler, IConnectionTimeoutHandler, IIdleTimeoutHandler {
