...
public class FlowControlTest {
	...
	public void testSimple() throws Exception {
		...
		while (con.available() > 0) { 
			Assert.assertFalse(con.isReadSuspended());  // external suspend mode is false (internal true)
	...
	private static final class ServerHandler implements IDataHandler {
		...
		public boolean onData(INonBlockingConnection connection) throws IOException, BufferUnderflowException, MaxReadSizeExceededException {
			...
			ReadableByteChannel in = new RandomAccessFile(QAUtil.getNameTestfile_400k(), "r").getChannel();
