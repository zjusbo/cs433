// $Id: ConnectedUdpTest.java 1108 2007-03-29 16:44:02Z grro $
/*
...
public final class ConnectedUdpTest {
		...
		for (int i = 0; i < countThreads; i++) {
			Thread t = new Thread() {
				...
				public void run() {
					runningThreads++;
					try {
						clientCall(10, 600, acceptorServer.getLocalAddress(), acceptorServer.getLocalPort());
					} catch (Exception e) {
						error = e;
					}
					runningThreads--;
	...
	private void clientCall(int callsCount, int exhangeDatasize, InetAddress acceptorAddress, int acceptorPort) throws Exception {
		...
		for (int i = 0; i < callsCount; i++) {
			byte[] dataRequestBytes = QAUtil.generatedByteArray(exhangeDatasize);
