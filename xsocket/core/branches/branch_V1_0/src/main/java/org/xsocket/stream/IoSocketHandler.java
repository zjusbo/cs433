// $Id: IoSocketHandler.java 1105 2007-03-29 15:33:43Z grro $
/*
...
final class IoSocketHandler extends IoHandler implements IHandle {
	...
	// event queue
	private final ProcessQueue taskQueue = new ProcessQueue();
	...
	static final class ProcessQueue {
		
		private final Queue<Runnable> tasks = new ConcurrentLinkedQueue<Runnable>(); 
		private final TaskProcessor taskProcessor = new TaskProcessor(this);
		
		public void processTask(IWorkerPool workerPool, Runnable newTask) {
	...
	private static final class TaskProcessor implements Runnable {
		
		private ProcessQueue processQueue = null;
		
		TaskProcessor(ProcessQueue processQueue) {
		...
		public void run() {
			...
			synchronized (processQueue) {
				Runnable task = processQueue.tasks.poll();
				if (task != null) {
					try {
						task.run();
					} catch (Exception e) {
