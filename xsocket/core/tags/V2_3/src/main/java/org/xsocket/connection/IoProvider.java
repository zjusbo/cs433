...
final class IoProvider  {
    ...
    private static Boolean suppressSncFlushWarning;
    ...
    static {
    	...
    	// coonection
    	suppressSncFlushWarning = readBooleanProperty(IoProvider.SUPPRESS_SYNC_FLUSH_WARNING_KEY, DEFAULT_SUPPRESS_SYNC_FLUSH_WARNING);
	...
	static boolean getSuppressSncFlushWarning() {
	    return suppressSncFlushWarning;
	}
	...
	public IoChainableHandler createClientIoHandler(SocketChannel channel) throws IOException {
		return createIoHandler(true, getGlobalClientDisptacherPool().nextDispatcher(0), channel, null, false);
	}
    ...
    public IoChainableHandler createSSLClientIoHandler(SocketChannel channel, SSLContext sslContext, boolean sslOn) throws IOException {
    	return createIoHandler(true, getGlobalClientDisptacherPool().nextDispatcher(0), channel, sslContext, sslOn);
    }
	...
	private static final class MBeanAdapter {
		...
		public int getConcurrentConnections() {
			return server.getNumberOfOpenConnections();
		}
		...
		public long getConnectionManagerWatchDogPeriodMillis() {
			return server.getConnectionManagerWatchDogPeriodMillis();
		}
		
		public int getConnectionManagerWatchDogRuns() {
			return server.getConnectionManagerWatchDogRuns();
		}
