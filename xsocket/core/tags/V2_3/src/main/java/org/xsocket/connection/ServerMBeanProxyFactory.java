...
final class ServerMBeanProxyFactory {
	...
	private static ObjectName registerMBeans(IServer server, String domain, String address) throws Exception {
		...
		if (hdl instanceof MBeanRegistration) {
			((MBeanRegistration) hdl).preRegister(ManagementFactory.getPlatformMBeanServer(), hdlObjectName);
	...
	private static void unregisterMBeans(IServer server, String domain, String address) throws Exception {
		address = address.replace(":", "_");
