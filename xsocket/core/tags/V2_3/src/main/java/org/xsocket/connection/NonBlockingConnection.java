...
public final class NonBlockingConnection extends AbstractNonBlockingStream implements INonBlockingConnection {
	...
	// sync flush support
	private final boolean isSuppressSncFlushWarning = IoProvider.getSuppressSncFlushWarning();
...
*    * @param waitForConnect        true, if the constructor should block until the connection is established. If the connect fails, the handler's onData and onDisconnect method will be called (if present) 
	 ...
	 */
	private NonBlockingConnection(InetSocketAddress remoteAddress, boolean waitForConnect, final int connectTimeoutMillis, final Map<String, Object> options, final SSLContext sslContext, final boolean isSecured,  IHandler appHdl, Executor workerpool, boolean autoflush, FlushMode flushmode) throws IOException {
		...
		final SocketChannel channel = openSocket(options);
		
		IoConnector connector = getDefaultConnector();
		
		// sync connect
		if (waitForConnect) {
		    SyncIoConnectorCallback callback = new SyncIoConnectorCallback(channel, sslContext, isSecured, connectionTimeoutMillis);
		    connector.connectAsync(channel, remoteAddress, connectTimeoutMillis, callback);
		    callback.connect();
		    if (LOG.isLoggable(Level.FINE)) {
		        LOG.fine("connection " + getId() + " established");
		    }
			
        // async connect			
		} else {
		    IIoConnectorCallback callback = new AsyncIoConnectorCallback(channel, sslContext, isSecured, connectionTimeoutMillis);
		    connector.connectAsync(channel, remoteAddress, connectTimeoutMillis, callback);
		}
	}

	
	private final class SyncIoConnectorCallback implements IIoConnectorCallback {
	    ...
	    public SyncIoConnectorCallback(SocketChannel channel, SSLContext sslContext, boolean isSecured, long connectTimeoutMillis) {
	    ...
	    public void onConnectionEstablished() throws IOException {
            register(channel, sslContext, isSecured);     
            notifyWaiting();
        }
        ...
        public void onConnectError(IOException ioe) {
            ...
            if (LOG.isLoggable(Level.FINE)) {
                LOG.fine("could not connect " + getId() + " " + ioe.toString());
            }
        ...
        public void onConnectTimeout() {
            this.ioe = new SocketTimeoutException("connect timeout " + DataConverter.toFormatedDuration(connectTimeoutMillis) +  " occured");
            
            if (LOG.isLoggable(Level.FINE)) {
                LOG.fine("commection timeout occured " + getId() + " " + ioe.toString());
            }
	...
	private final class AsyncIoConnectorCallback implements IIoConnectorCallback {
        ...
        public AsyncIoConnectorCallback(SocketChannel channel, SSLContext sslContext, boolean isSecured, long connectTimeoutMillis) {
            this.channel = channel;
            this.sslContext = sslContext;
            this.isSecured = isSecured;
            this.connectTimeoutMillis = connectTimeoutMillis;
        ...
        public void onConnectTimeout() {
            onConnectException(new SocketTimeoutException("connect timeout " + DataConverter.toFormatedDuration(connectTimeoutMillis) +  " occured"));
        }
    }
	...
	protected boolean reset() {
		try {
			...
			isWriteCompletionSupportActivated = false;
	...
	private void onPostData() {
		...
		try {
			handlerAdapter.get().onData(NonBlockingConnection.this, taskQueue);
		} catch (IOException ioe) {
			if (LOG.isLoggable(Level.FINE)) {
				LOG.fine("error occured by performing onData callback on " + handlerAdapter.get() + " " + ioe.toString());
			}
		}
	}
	...
	private void onConnect() {
		try {
			...
			if (LOG.isLoggable(Level.FINE)) {
				LOG.fine("error occured by performing onConnect callback on " + handlerAdapter.get() + " " + ioe.toString());
			}
		}
	}
	...
	private void onDisconnect() {
		...
		if (!disconnectOccured) {
			...
			// then onDisconnect
			try {
				handlerAdapter.get().onDisconnect(NonBlockingConnection.this, taskQueue);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("error occured by performing onDisconnect callback on " + handlerAdapter.get() + " " + ioe.toString());
				}
			}
		}
	}
	...
	private void onConnectException(IOException ioe) {
        ...
        if (!connectExceptionOccured) {
            connectExceptionOccured = true;
            
            try {
                handlerAdapter.get().onConnectException(NonBlockingConnection.this, taskQueue, ioe);
            } catch (IOException e) {
                if (LOG.isLoggable(Level.FINE)) {
                    LOG.fine("error occured by performing onDisconnect callback on " + handlerAdapter.get() + " " + e.toString());
                }
            }
        }
    }
	...
	void onIdleTimeout() {
		if (!idleTimeoutOccured) {
			...
			try {
				...
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("error occured by performing onIdleTimeout callback on " + handlerAdapter.get() + " " + ioe.toString());
				}
			}
		} else {
	...
	private void onConnectionTimeout() {
		if (!connectionTimeoutOccured) {
			...
			try {
				...
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("error occured by performing onConnectionTimeout callback on " + handlerAdapter.get() + " " + ioe.toString());
				}
			}
		} else {
...
/*	public final void setReadTransferRate(int bytesPerSecond) throws ClosedChannelException, IOException {
    ...
    public void write(ByteBuffer buffer, IWriteCompletionHandler writeCompletionHandler) throws IOException {
        ...
        registerCompletionHandler(writeCompletionHandler, buffer);
        write(buffer);
    ...
    public void write(ByteBuffer[] buffers, IWriteCompletionHandler writeCompletionHandler) throws IOException {
        ...
        registerCompletionHandler(writeCompletionHandler, buffers);
        write(buffers);
    ...
    public void write(List<ByteBuffer> buffers, IWriteCompletionHandler writeCompletionHandler) throws IOException {
        ...
        registerCompletionHandler(writeCompletionHandler, buffers.toArray(new ByteBuffer[buffers.size()]));
        write(buffers);
    ...
    private void registerCompletionHandler(IWriteCompletionHandler writeCompletionHandler, ByteBuffer... buffersToWrite) {
    ...
    private final class SendTask implements IWriteCompletionHandler, IUnsynchronized {
	...
	private void internalFlush() throws ClosedChannelException, IOException {
		...
		if (getFlushmode() == FlushMode.SYNC) {
			syncFlush();  //flush write queue by using a write guard to wait until the onWrittenEvent has been occurred
		} else {
		    asyncFlush();
		}
     ...
     * async flush
     */
    private void asyncFlush() throws ClosedChannelException, IOException {
	...
	private void syncFlush() throws ClosedChannelException, IOException {
		...
		if (isSuppressSncFlushWarning && ConnectionUtils.isDispatcherThread()) {
	...
	private static SocketChannel openSocket(Map<String ,Object> options) throws IOException {
	...
	private final class WriteCompletionHolder implements Runnable {
	    ...
	    void performOnWritten() {
	        ...
	        } else if (handlerInfo.isOnWrittenMultithreaded()) {
	            taskQueue.performMultiThreaded(this, getWorkerpool());
	            
	        } else {
	            taskQueue.performNonThreaded(this);
	        }
	    }
	    ...
	    void performOnException(final IOException ioe) {
	        ...
	        } else if (handlerInfo.isOnExceptionMutlithreaded()) {
