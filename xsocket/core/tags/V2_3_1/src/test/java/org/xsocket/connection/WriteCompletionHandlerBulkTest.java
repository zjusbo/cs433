...
public final class WriteCompletionHandlerBulkTest {
	...
	public void testClientSideAsyncUpload() throws Exception {
	    ...
	    ChannelAsyncWriter hdl = new ChannelAsyncWriter(filename, con);
	    hdl.onWritten(0);
        ...
        } while (available < new File(filename).length());

	    
	    
	    ByteBuffer[] data = con.readByteBufferByLength((int) new File(filename).length());
	    Assert.assertTrue(QAUtil.isEquals(new File(filename), data));
    ...
    public void testClientSideAsyncUploadNonThreaded() throws Exception {
        ...
        NonThreadedChannelAsyncWriter hdl = new NonThreadedChannelAsyncWriter(filename, con);
        hdl.onWritten(0);
