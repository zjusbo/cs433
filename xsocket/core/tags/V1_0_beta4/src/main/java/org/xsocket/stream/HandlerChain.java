// $Id: HandlerChain.java 900 2007-02-11 14:00:33Z grro $
 ...
 */
public final class HandlerChain implements  IHandler, IConnectHandler, IDisconnectHandler, IDataHandler, ITimeoutHandler, IConnectionScoped, ILifeCycle {
	...
	public void onInit() {
		...
		for (Integer pos : lifeCycleChain) {		
			((ILifeCycle) handlers[pos]).onInit();
		}
	}
	...
	public void onDestroy() {
		for (Integer pos : lifeCycleChain) {
			((ILifeCycle) handlers[pos]).onDestroy();
		}
	}
	...
	public boolean onDisconnect(String connectionId) throws IOException {
		for (Integer pos : disconnectHandlerChain) {
			boolean result = ((IDisconnectHandler) handlers[pos]).onDisconnect(connectionId);
			if (result == true) {
				return true;
			}
		}
