...
public abstract class AbstractConnection implements IConnection {
		...
		if (isOpen() && isReceiving) {
			...
			switch (read) {
				...
				default:
					...
					if (LOG.isLoggable(Level.FINE)) {
						LOG.fine("[" + getId() + "] received: " + printByteBuffersForDebug(new ByteBuffer[] { result }));
					}
					break;
			}			
		} 				
	 ...
	 */
	protected final synchronized void addToReceiveQueue(ByteBuffer buffer) {
	...
	private String printByteBuffersForDebug(ByteBuffer[] buffers) {
		...
		// then handle text/binary data
		String result;
		try {
			result = TextUtils.toString(copies, "UTF-8");
		} catch (Exception e) {
			StringBuffer sb = new StringBuffer();
			for (ByteBuffer buffer : copies) {
				while (buffer.hasRemaining()) {
					sb.append(Integer.toHexString((int) buffer.get()) + " ");
				}
			}
			result = sb.toString();
		}
