...
public final class SimpleFileChannelPerformanceTest  {
	...
	private static final String FILE = QAUtil.getNameTestfile_400k(); 
	...
	public void testTransferConnectionToFileChannel() throws Exception {
		...
		// warm up
		load(con1, length, 100);
		QAUtil.sleep(300);
		
		load(con2, length, 100);
		QAUtil.sleep(300);
		
		
		long elapsed1 = load(con1, length, 300);
		QAUtil.sleep(300);
		
		long elapsed2 = load(con2, length, 300);
		
		
		int p = (int) (elapsed2 * 100 / elapsed1);
		System.out.println("elapsed fc controlled " + elapsed1 + " millis, elapsed connection controlled " + 
				           elapsed2 + " millis (" + p +"%)");
		...
		Assert.assertTrue("connection.transferFrom(fc) should be faster than fc.transferTo(0, fc.size(), connection)", elapsed2 < elapsed1);
	}	
	...
	private static final class UploadHandler2 implements IConnectHandler, IDataHandler {
