...
final class SSLProcessor {
	 ...
	 * @throws ClosedException if the connection is already closed
	 */
	synchronized void decrypt(ByteBuffer[] encryptedBufferList) throws IOException ,ClosedException {
	 ...
	 * @throws ClosedException if the connection is already closed
	 */
	void encrypt(ByteBuffer[] plainBufferList) throws ClosedException, IOException{
	 ...
	 * @throws ClosedException if the connection is already closed
	 */
	void encrypt() throws ClosedException, IOException{
		ByteBuffer outAppData = ByteBuffer.allocate(0); 
		wrap(outAppData);
	}

	
	private synchronized ArrayList<ByteBuffer> unwrap(ByteBuffer inNetData) throws SSLException, ClosedException, IOException {
	...
	private synchronized void wrap(ByteBuffer outAppData) throws SSLException, ClosedException, IOException {
		...
		do {
		    ...
		    if ((engineResult.getStatus() == SSLEngineResult.Status.OK) 
				...
				memoryManager.recycleMemory(outNetData);
				throw new ClosedException("Couldn't wrap, because connection is closed");
			}
