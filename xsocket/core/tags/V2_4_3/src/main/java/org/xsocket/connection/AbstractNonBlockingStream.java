...
public abstract class AbstractNonBlockingStream implements WritableByteChannel, Closeable {
	...
	private static final int TRANSFER_BYTE_BUFFER_MAX_MAP_SIZE = IoProvider.getTransferByteBufferMaxSize();
	...
	private void ensureStreamIsOpenAndWritable() throws ClosedChannelException {
		if (!isOpen.get()) {
			throw new ExtendedClosedChannelException("could not write. Channel is closed (read buffer size=" + readQueue.getSize() + ")");
		}		
