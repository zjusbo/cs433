...
final class IoSocketHandler extends IoChainableHandler {
	...
	private AtomicBoolean isDetached = new AtomicBoolean(true);
	...
	private AtomicLong countIncompleteWrites = new AtomicLong(0);
	private AtomicLong countInitWrite = new AtomicLong(0);
	private AtomicLong countIgnoreWriteSocketBecauseItisClosed = new AtomicLong(0);
	private AtomicLong countWriteSocket = new AtomicLong(0);
	private AtomicLong countDirectWriteTaskPerformed = new AtomicLong(0);
	private AtomicLong countMergingWriteTaskPerformed = new AtomicLong(0);
	...
	void setDetached(boolean isDetached) {
		this.isDetached.set(isDetached);
	}
	...
	public void flush() throws IOException {
		countInitWrite.incrementAndGet();
	...
	private void realClose() {
		...
		boolean disconnect = isDisconnect.getAndSet(true);
		if (!disconnect) {
			try {
		...
		if (isOpen()) {
			...
			switch (read) {
				...
				default:					
					...
					if (LOG.isLoggable(Level.FINE)) {
	 	 				LOG.fine("[" + id + "] received (" + (dataBuffer.limit() - dataBuffer.position()) + " bytes, total " + (receivedBytes.get() + read) + " bytes): " + DataConverter.toTextOrHexString(new ByteBuffer[] {dataBuffer.duplicate() }, "UTF-8", MAXSIZE_LOG_READ));
		 			}
	...
	private void writeSocket() throws IOException {
		...
		if (isOpen()) {
			countWriteSocket.incrementAndGet();
			...
			// no, create a new one
				...
				isWriting.set(true);  // set write flag to support Method hasMoreDataToWrite()
				try {
			...
			if (result.isAllWritten()) {
				isWriting.set(false);  // remove write flag
				writeTask.release();
			...
			countIgnoreWriteSocketBecauseItisClosed.incrementAndGet();
			if (LOG.isLoggable(Level.FINEST)) {
				if (!isOpen()) {
	...
	private boolean hasMoreDataToWrite() {
		return (!sendQueue.isEmpty()) || isWriting.get();
	}
	...
	public String getInfo() {
		return "countIncompleteWrites=" + countIncompleteWrites.get() + ", countInitWrite=" + countInitWrite.get() +
		       ", countIgnoreWriteSocketBecauseItisClosed=" + countIgnoreWriteSocketBecauseItisClosed.get() +
		       ", countWriteSocket=" + countWriteSocket.get() + ", countDirectWriteTaskPerformed=" + countDirectWriteTaskPerformed.get() +
		       ", countMergingWriteTaskPerformed=" + countMergingWriteTaskPerformed.get(); 
	}
	...
	public String toString() {
   	...
   	private static final class TaskFactory {
   		...
   		static IWriteTask newTask(IoQueue sendQueue, int soSendBufferSize) {
   			...
   			ByteBuffer[] buffersToWrite = sendQueue.drain(soSendBufferSize);
    ...
    private static final class MergingWriteTask implements IWriteTask {
        ...
        public IWriteResult write(IoSocketHandler handler) throws IOException {
            ...
            try {
            	...
            	handler.countMergingWriteTaskPerformed.incrementAndGet();
                ...
                if (!writeBuffer.hasRemaining()) {
                	...
                	handler.countIncompleteWrites.incrementAndGet();
 		...
 		private boolean isReusable = true;
   		...
   		public IWriteResult write(IoSocketHandler handler) throws IOException {
   			...
   			try {
   				...
   				handler.countDirectWriteTaskPerformed.incrementAndGet();
