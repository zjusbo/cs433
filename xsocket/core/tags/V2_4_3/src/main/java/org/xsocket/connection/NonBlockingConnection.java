...
public final class NonBlockingConnection extends AbstractNonBlockingStream implements INonBlockingConnection {
	...
	private void onPostData() {
		...
		try {
			handlerAdapter.get().onData(NonBlockingConnection.this, taskQueue, false);
		} catch (IOException ioe) {
			if (LOG.isLoggable(Level.FINE)) {
	...
	private void onDisconnect() {
		...
		if (!disconnectOccured) {
			...
			// call first onData
			try {
				handlerAdapter.get().onData(NonBlockingConnection.this, taskQueue, true);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("[" + getId() + "] error occured by performing onData callback on " + handlerAdapter.get() + " " + ioe.toString());
				}
...
/*	public final void setReadTransferRate(int bytesPerSecond) throws ClosedChannelException, IOException {
	...
	private void internalFlush() throws ClosedChannelException, IOException {
		...
		if (!isWriteBufferEmpty()) {
			...
			// sync flush mode?
    		if (getFlushmode() == FlushMode.SYNC) {
    			synchronWriter.syncWrite();
    			
    		// ... no, async
    		} else {
    			ioHandler.write(drainWriteQueue());
    		    ioHandler.flush();
    		}
		}
	...
	private final class SynchronWriter {
		...
		private final AtomicBoolean hasPendingBuffers = new AtomicBoolean(false);
		...
		boolean reset() {
			synchronized (this) {
				boolean isReusable = pendingBuffers.isEmpty();
				clear();
		...
		void syncWrite() throws IOException, SocketTimeoutException {

			assert (hasPendingBuffers.get() == false) : "nor pending buffers should exists";
			assert (ioe == null) : "io exception should be null";				
			...
			synchronized (this) {
				...
				try {					
					...
					while (true) {		
						 ...
						 // all data written?
		                 if (hasPendingBuffers.get() == false) {
		                	 if (LOG.isLoggable(Level.FINE)) {
...
/*	
	final class SyncWriter {
	    ...
	    private int countWrittenBuffers = 0;
	    private int countTotalNotifiedBuffers = 0;
	    private int countMissedNotifiedBuffers = 0;
	    
	    
	    void flushAndWaitUntilWritten() throws IOException {
	        ...
	        synchronized (this) {
	            try {
	                ...
	                if (buffers != null) {
		                ...
		                ioHandler.write(buffers);
		                countWrittenBuffers += buffers.length;
		                ...
		                throw new SocketTimeoutException("send timeout " + DataConverter.toFormatedDuration(sendTimeoutMillis) + " reached. returning from sync flushing" + 
		                		                         " (pending buffers= " + pendingWriteConfirmations.size() + 
		                		                         ", countWrittenBuffers=" + countWrittenBuffers + ", countTotalNotifiedBuffers=" + countTotalNotifiedBuffers +
		                		                         ", countMissedNotifiedBuffers=" + countMissedNotifiedBuffers + ")");
	                } 
	    ...
	    void onWritten(ByteBuffer data) {
	        ...
	        if (data == null) {
	        	...
	        	synchronized (this) {
		            for (ByteBuffer buf : pendingWriteConfirmations) {
	                    if (buf == data) {
	                        boolean found = pendingWriteConfirmations.remove(data);
	                        if (!found) {
	                        	countMissedNotifiedBuffers++;
	                        }
	                        break;
	                    }
	                }
        ...
        synchronized boolean reset() {
            return pendingWriteConfirmations.isEmpty();
        }
