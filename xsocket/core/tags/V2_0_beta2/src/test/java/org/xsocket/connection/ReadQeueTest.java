...
*/
public final class ReadQeueTest {
	...
	public void testFind() throws Exception {
		...
		ReadQueue readQueue = new ReadQueue();
		readQueue.append(new ByteBuffer[] { input });
		...
		readQueue.append(new ByteBuffer[] { input2 });
	...
	private static final void read(ReadQueue readQueue, int run) throws IOException {
	...
	private static final void insert(int count, ReadQueue readQueue) {
		for (int i = 0; i < count; i++) {
			ByteBuffer buf = DataConverter.toByteBuffer("TEST", "UTF-8");
			readQueue.append(new ByteBuffer[] { buf });
			ByteBuffer buf2 = DataConverter.toByteBuffer(";", "UTF-8");
			readQueue.append(new ByteBuffer[] { buf2 });
			ByteBuffer buf3 = DataConverter.toByteBuffer((int) 5);
			readQueue.append(new ByteBuffer[] { buf3 });
		}
	}
