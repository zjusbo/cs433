 ...
 */
final class IoThrottledReadHandler extends ChainableIoHandler {
	 ...
	 */
	IoThrottledReadHandler(ChainableIoHandler successor) {
		super(successor);
		
		readControlTask = new ReadControlTask(this);
		DefaultIoProvider.getTimer().schedule(readControlTask, CHECK_PERIOD_MILLIS, CHECK_PERIOD_MILLIS);
	}
	...
	public void init(IIoHandlerCallback callbackHandler) throws IOException {
		...
		orgReadBufferSize = (Integer) getSuccessor().getOption(IClientIoProvider.SO_RCVBUF);
	...
	public boolean reset() {
		...
		try {
			getSuccessor().setOption(IClientIoProvider.SO_RCVBUF, orgReadBufferSize);
	...
	void setReadRateSec(int readRateSec) throws IOException {
		...
		if (readRateSec < orgReadBufferSize) {
			getSuccessor().setOption(IClientIoProvider.SO_RCVBUF, readRateSec);
		} else {
			getSuccessor().setOption(IClientIoProvider.SO_RCVBUF, orgReadBufferSize);
		}
	...
	private IoSocketHandler getSocketHandler() {
		ChainableIoHandler successor = this;
		do {
