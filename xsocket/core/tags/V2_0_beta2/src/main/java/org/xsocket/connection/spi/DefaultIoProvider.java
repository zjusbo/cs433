...
public final class DefaultIoProvider implements IClientIoProvider, IServerIoProvider {
     ...
     * {@inheritDoc}
     */
    public IIoHandler setReadTransferRate(IIoHandler ioHandler, int bytesPerSecond) throws IOException {

    	// unlimited? remove throttling handler if exists
    	if (bytesPerSecond == UNLIMITED) {
    		IoThrottledReadHandler delayReader = (IoThrottledReadHandler) getHandler((ChainableIoHandler) ioHandler, IoThrottledReadHandler.class);
    		if (delayReader != null) {
    			delayReader.reset();
    			ChainableIoHandler successor = delayReader.getSuccessor();
    			return successor;
    		} else {
    			return ioHandler;
    		}
       	...
       	// ...no -> add throttling handler if not exists and set rate
			...
			IoThrottledReadHandler delayReader = (IoThrottledReadHandler) getHandler((ChainableIoHandler) ioHandler, IoThrottledReadHandler.class);
			if (delayReader == null) {
				delayReader = new IoThrottledReadHandler((ChainableIoHandler) ioHandler);
			}

			delayReader.setReadRateSec(bytesPerSecond);
			
			delayReader.init(((ChainableIoHandler) ioHandler).getPreviousCallback());
			return delayReader;
    	}
	}
	...
	private static final class DispatcherPoolListener implements IAcceptorListener {
		...
		public void onDispatcherAdded(IDispatcher dispatcher) {
			...
			try {
				ObjectName objectName = new ObjectName(domain + ".server." + address + ":type=xDispatcher,name=" + dispatcher.hashCode());
