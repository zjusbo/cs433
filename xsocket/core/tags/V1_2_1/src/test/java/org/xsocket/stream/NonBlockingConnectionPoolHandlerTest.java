...
public final class NonBlockingConnectionPoolHandlerTest {
	...
	public void testDataHandler() throws Exception {
		...
		final IMultithreadedServer server = new MultithreadedServer("localhost", 9977, new EchoHandler());
				...
				public void run() {
					...
					try {
						...
						for (int i = 0; i < 10; i++) {
							...
							INonBlockingConnection con = pool.getNonBlockingConnection(InetAddress.getByName("localhost"), server.getLocalPort(), clientHdl);
			...
			Thread t = new Thread() {
				...
				public void run() {
					...
					try {
						...
						for (int i = 0; i < 2; i++) {
							ClientHandler clientHdl = new ClientHandler();
							INonBlockingConnection connection = pool.getNonBlockingConnection(InetAddress.getByName("localhost"), server.getLocalPort(), clientHdl);
