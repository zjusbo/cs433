// $Id: LargeDataTransferTest.java 764 2007-01-15 06:26:17Z grro $
/*
...
public final class LargeDataTransferTest  {
	...
	@Test public void testData() throws Exception {
		...
		send("127.0.0.1", server.getLocalPort(), 700000);
		System.gc();
	...
	private void send(String host, int port, int dataSizeInBytes) throws IOException{
		...
		IBlockingConnection connection = new BlockingConnection(host, port, 524288);
		...
		for (int i = 0; i < 1; i++) {
			...
			ByteBuffer[] response = connection.receiveByteBufferByDelimiter(EchoHandler.DELIMITER);
