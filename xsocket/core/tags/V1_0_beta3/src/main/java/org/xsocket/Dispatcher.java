/// $Id: Dispatcher.java 774 2007-01-15 08:09:57Z grro $
/*
...
public final class Dispatcher<T extends IHandle> implements IDispatcher<T>, Runnable {
	...
	public void register(T handle) throws IOException {
		...
		synchronized (dispatcherThreadGuard) {
			...
			handle.getChannel().register(selector, SelectionKey.OP_READ, handle);
		}
	...
	/**
	 * announce a write need based for he given handle. <br><br>
	 * 
	...
	/**
	 * chech if this dispatcher is running
	 * @return true, if the disptacher is running
	 */
