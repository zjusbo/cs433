// $Id: IoSocketDispatcherPool.java 1058 2007-03-21 18:31:37Z grro $
/*
...
final class IoSocketDispatcherPool {
	...
	IoSocketDispatcherPool(int preallocationSize, IWorkerPool workerPool) {
	...
	synchronized void setWorkerPool(IWorkerPool workerPool) {
		this.workerPool = workerPool;
	}
			...
			if (currentRunning != size) {
				if (currentRunning > size) {
					...
					for (int i = currentRunning; i < size; i++) {
						IoSocketDispatcher dispatcher = new IoSocketDispatcher(new SingleThreadedMemoryManager(), workerPool);
						dispatchers.addLast(dispatcher);
		
						Thread t = new Thread(dispatcher);
						t.setDaemon(false);
						t.setName("Dispatcher#" + i);
						t.start();
