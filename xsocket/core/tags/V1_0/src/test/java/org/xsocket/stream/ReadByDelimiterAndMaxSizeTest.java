// $Id: ReadByDelimiterAndMaxSizeTest.java 1019 2007-03-15 08:06:45Z grro $
/*
...
public final class ReadByDelimiterAndMaxSizeTest {
	...
	@Test public void testBlocking1() throws Exception {
	...
	@Test public void testBlocking2() throws Exception {
	...
	@Test public void testNonBlockingClientSide() throws Exception {
		IMultithreadedServer server = new MultithreadedServer(new ServerHandler2());
		new Thread(server).start();
	...
	@Test public void testNonBlockingServerSide() throws Exception {
		ServerHandler3 hdl = new ServerHandler3();
		IMultithreadedServer server = new MultithreadedServer(hdl);
		new Thread(server).start();

		IBlockingConnection connection = new BlockingConnection(server.getLocalAddress(), server.getLocalPort());
		connection.write(TestUtil.generatedByteArray(MAX_READ_SIZE + 10));
