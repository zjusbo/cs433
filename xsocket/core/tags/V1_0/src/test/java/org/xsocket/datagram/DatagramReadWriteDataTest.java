// $Id: DatagramReadWriteDataTest.java 1042 2007-03-20 17:57:07Z grro $
/*
...
public final class DatagramReadWriteDataTest {
	...
	@Test public void testDataTypes() throws Exception {		
		...
		IConnectedEndpoint clientEndpoint = new ConnectedEndpoint(serverEndpoint.getLocalSocketAddress());
		...
		byte[] data = TestUtil.generatedByteArray(10);
		packet.write(data);
		byte[] data2 = TestUtil.generatedByteArray(17);
		...
		byte[] data3 = TestUtil.generatedByteArray(4);
		packet.write(data3);
		...
		Assert.assertTrue(TestUtil.isEquals(serverSidePacket.readBytesByLength(data.length), data));
		Assert.assertTrue(TestUtil.isEquals(serverSidePacket.readByteBufferByLength(data2.length), buffer2));
		Assert.assertTrue(TestUtil.isEquals(serverSidePacket.readByteBufferByLength(data2.length), buffer2));
		...
		Assert.assertTrue(TestUtil.isEquals(serverSidePacket.readBytesByLength(4), data3));
		try {
	...
	@Test public void testString() throws Exception {		
		...
		UserDatagram packet = new UserDatagram(serverEndpoint.getLocalAddress().getHostName(), serverEndpoint.getLocalPort(), msg.getBytes().length + msg2.getBytes().length);
	...
	@Test public void testMaxReadSize() throws Exception {		
		Handler hdl = new Handler();
		IEndpoint serverEndpoint = new Endpoint(SIZE, hdl);
		IEndpoint clientEndpoint = new Endpoint();
		
		UserDatagram packet = new UserDatagram(serverEndpoint.getLocalAddress().getHostName(), serverEndpoint.getLocalPort(), SIZE);
		packet.write(TestUtil.generatedByteArray(10));
