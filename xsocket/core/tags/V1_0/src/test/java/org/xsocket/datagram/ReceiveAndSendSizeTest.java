// $Id: ReceiveAndSendSizeTest.java 1042 2007-03-20 17:57:07Z grro $
/*
...
public final class ReceiveAndSendSizeTest {
	...
	@Test public void testMulticastEndpointSmallerSize() throws Exception {
		...
		for (int i = 0; i < 6; i++) {
			byte[] data = TestUtil.generatedByteArray(packageSize - 10);
...
@Test public void testMulticastEndpointLargerSize() throws Exception {
		...
		for (int i = 0; i < 6; i++) {
			...
			Assert.assertTrue(TestUtil.isEquals(reducedData, handler.lastReceived.readBytes()));
		}
	...
	@Test public void testNonBlockingEndpoint() throws Exception {
		...
		for (int i = 0; i < 6; i++) {
			...
			DatagramPacket requestPackage = new DatagramPacket(data, data.length, new InetSocketAddress(serverEndpoint.getLocalAddress(), serverEndpoint.getLocalPort()));
	...
	@Test public void testNonBlockingEndpointSmallerSize() throws Exception {
		...
		for (int i = 0; i < 6; i++) {
			byte[] data = TestUtil.generatedByteArray(packageSize - 6);
			UserDatagram packet = new UserDatagram(serverEndpoint.getLocalSocketAddress(), data);
	...
	@Test public void testNonBlockingEndpointLargerSize() throws Exception {
