// $Id: TimeoutTest.java 1024 2007-03-17 11:34:26Z grro $
/*
...
public final class TimeoutTest {
	...
	@Test public void testIdleTimeout() throws Exception {
		...
		Assert.assertTrue(server.getIdleTimeoutSec() == 2);
		...
		Assert.assertTrue("idle timeout should have been occured", idleTimeoutOccured);
	...
	@Test public void testIdleTimeoutDefault() throws Exception {
		...
		INonBlockingConnection con = new NonBlockingConnection(server.getLocalAddress(), server.getLocalPort(), clientHdl);
		con.write("rt");
		con.flush();
	...
	@Test public void testIdleTimeoutDefault2() throws Exception {
		IMultithreadedServer server = new MultithreadedServer(new EchoHandler());
		new Thread(server).start();
