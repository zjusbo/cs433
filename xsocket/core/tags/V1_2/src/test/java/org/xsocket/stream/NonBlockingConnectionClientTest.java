...
public final class NonBlockingConnectionClientTest {
	...
	public void testLive() throws Exception {
		INonBlockingConnection connection = new NonBlockingConnection("www.web.de", 80);
	...
	@Test 
	public void testLiveWithHandler() throws Exception {
		//QAUtil.setLogLevel(Level.FINE);

		ClientHandler hdl = new ClientHandler();
		INonBlockingConnection connection = new NonBlockingConnection("www.web.de", 80, hdl);
		connection.setAutoflush(true);
		...
		Assert.assertTrue(hdl.response.indexOf("WEB.DE") != -1);
	...
	public void testSimple() throws Exception {
		...
		SendTask sendTask = new SendTask(testServer.getLocalAddress(), testServer.getLocalPort(), "testi@xsocket.de");
		do {
			sendTask.perform();
	...
	public void testDelayedWrite() throws Exception {
		...
		INonBlockingConnection connection = new NonBlockingConnection(testServer.getLocalAddress(), testServer.getLocalPort());
		connection.setAutoflush(true);
		...
		Assert.assertTrue(inTimeRange(elapsed, 0, 0, 700)); 
		...
		Assert.assertTrue(isEquals(data, response));
		Assert.assertTrue(inTimeRange(elapsed, 3333, 2000, 5500));  // 5000 byte / 1500 bytesPerSec -> 3,3 sec
