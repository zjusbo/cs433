...
public final class BulkDestructiveTestMailSender {
	...
	public static void main(final String[] args)  {
		...
		for (int i = 0; i < size; i++) {
			Thread t = new Thread() {
				public void run() {
					try {
						DestructiveMailSender sender = new DestructiveMailSender();
						while (true) {
							String errorMessage = sender.send(args[0], Integer.parseInt(args[1]));
