...
final class IoProvider  {
	...
	private static final String DEFAULT_IS_UNREGISTERED_WRITE_ALLOWED = "true";
    ...
    static {
    	...
    	unregisteredWriteAllowed = readBooleanProperty(IS_UNREGISTERED_WRITE_ALLOWED, DEFAULT_IS_UNREGISTERED_WRITE_ALLOWED);
	...
	public ObjectName registerMBeans(Server server, IoAcceptor acceptor, String domain, String address) throws JMException {
		...
		if (acceptor instanceof IoAcceptor) {
			...
			throw new JMException("only acceptor of instance " + IoAcceptor.class.getName() + " is supported, not " + acceptor.getClass().getName());
		}
	}
