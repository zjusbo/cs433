...
abstract class AbstractMemoryManager { 
	...
	protected final ByteBuffer newBuffer(int size) throws IOException {
		...
		try {
			...
			String msg = null;
			if (useDirectMemory) {
				msg = "out of memory exception occured by trying to allocated direct memory " + oome.toString();
			} else {
				msg = "out of memory exception occured by trying to allocated non-direct memory " + oome.toString();
			}
