...
public final class WaitForConnectTest {
	...
	public void testNonWait() throws Exception {
		...
		INonBlockingConnection con = new NonBlockingConnection(InetAddress.getByName("199.9.9.9"), 999, hdl, false, 1000);
		
		QAUtil.sleep(1500);
		
		Assert.assertEquals(0, hdl.onConnectCalled);
		Assert.assertEquals(1, hdl.onDataCalled);
		Assert.assertEquals(1, hdl.onDisconnectCalled);
		Assert.assertEquals(-1, hdl.getAvailable());
	}
