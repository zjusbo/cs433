...
abstract class AbstractConnectionPool implements IConnectionPool {
	 ...
	 */
	synchronized final PoolableConnection getConnection(InetSocketAddress address, Executor workerPool, int connectTimeoutMillis) throws IOException, WaitTimeoutException {
		...
		if (isOpen) {
			...
			if (poolableConnection == null) {
				...
				if (maxWaitMillis == NULL) {
					poolableConnection = newConnection(address, workerPool, connectTimeoutMillis);
					if (LOG.isLoggable(Level.FINE)) {
	...
	private PoolableConnection newConnection(InetSocketAddress address, Executor workerPool, int connectTimeoutMillis) throws IOException {
		...
		do {
			...
			try {
				...
				connection = createConnection(address, workerPool, connectTimeoutMillis);
				return connection;
	 ...
	 */
	abstract PoolableConnection createConnection(InetSocketAddress address, Executor workerPool, int connectTimeoutMillis) throws IOException;
	...
	abstract class PoolableConnection implements IConnection {
		...
		public ByteBuffer[] readByteBufferByDelimiter(String delimiter, String encoding) throws IOException, ClosedConnectionException, MaxReadSizeExceededException {
			return delegee.readByteBufferByDelimiter(delimiter, encoding);
		}
		...
		public byte[] readBytesByDelimiter(String delimiter, String encoding) throws IOException, ClosedConnectionException, MaxReadSizeExceededException {
			return delegee.readBytesByDelimiter(delimiter, encoding);
		}
