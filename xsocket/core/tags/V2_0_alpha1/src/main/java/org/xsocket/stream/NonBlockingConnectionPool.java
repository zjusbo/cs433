...
public final class NonBlockingConnectionPool extends AbstractConnectionPool {
	...
	public INonBlockingConnection getNonBlockingConnection(String host, int port) throws IOException, WaitTimeoutException {
		return (INonBlockingConnection) getConnection(new InetSocketAddress(host, port), null, Integer.MAX_VALUE);
	}
	...
	public INonBlockingConnection getNonBlockingConnection(String host, int port, int connectTimeoutMillis) throws IOException, WaitTimeoutException {
		return (INonBlockingConnection) getConnection(new InetSocketAddress(host, port), null, connectTimeoutMillis);
	}
	...
	public INonBlockingConnection getNonBlockingConnection(InetAddress address, int port) throws IOException, WaitTimeoutException {
		return (INonBlockingConnection) getConnection(new InetSocketAddress(address, port), null, Integer.MAX_VALUE);
	}
	...
	public INonBlockingConnection getNonBlockingConnection(InetAddress address, int port, int connectTimeoutMillis) throws IOException, WaitTimeoutException {
		return (INonBlockingConnection) getConnection(new InetSocketAddress(address, port), null, connectTimeoutMillis);
	}
	...
	public INonBlockingConnection getNonBlockingConnection(InetAddress address, int port, IHandler appHandler) throws IOException, WaitTimeoutException {
		return getNonBlockingConnection(address, port, appHandler, null, Integer.MAX_VALUE);
	}
	...
	public INonBlockingConnection getNonBlockingConnection(String host, int port, IHandler appHandler) throws IOException, WaitTimeoutException {
		return (INonBlockingConnection) getConnection(new InetSocketAddress(host, port), appHandler, null, Integer.MAX_VALUE);
	}
	...
	public INonBlockingConnection getNonBlockingConnection(InetAddress address, int port, IHandler appHandler, int connectTimeoutMillis) throws IOException, WaitTimeoutException {
		return getNonBlockingConnection(address, port, appHandler, null, connectTimeoutMillis);
	}
	...
	public INonBlockingConnection getNonBlockingConnection(InetAddress address, int port, IHandler appHandler, Executor workerPool, int connectTimeoutMillis) throws IOException, WaitTimeoutException {
		return (INonBlockingConnection) getConnection(new InetSocketAddress(address, port), appHandler, workerPool, connectTimeoutMillis);
	}
	...
	private INonBlockingConnection getConnection(InetSocketAddress address, IHandler appHandler, Executor workerPool, int connectTimeoutMillis) throws IOException, WaitTimeoutException {
		PoolableNonBlockingConnection connection = (PoolableNonBlockingConnection) getConnection(address, workerPool, connectTimeoutMillis);
		if (connection != null) {
	 ...
	 */
	@Override
	PoolableConnection createConnection(InetSocketAddress address, Executor workerPool, int connectTimeoutMillis) throws IOException {
		if (workerPool != null) {
	...
	private final class PoolableNonBlockingConnection extends PoolableConnection implements INonBlockingConnection {
		...
		public PoolableNonBlockingConnection(InetSocketAddress address, int connectTimeoutMillis) throws IOException {
			super(NonBlockingConnectionPool.this, new NonBlockingConnection(address.getAddress(), address.getPort(), connectTimeoutMillis, new HandlerProxy()), address);
		...
		public List<String> readAvailableStringsByDelimiter(String delimiter, String encoding) throws IOException, UnsupportedEncodingException {
			return getUnderlyingConnection().readAvailableStringsByDelimiter(delimiter, encoding);
		}

		public TransferResult transferAvailableByDelimiter(String delimiter, WritableByteChannel outputChannel) throws IOException, ClosedConnectionException {
			return getUnderlyingConnection().transferAvailableByDelimiter(delimiter, outputChannel);
		}

		public TransferResult transferAvailableByDelimiter(String delimiter, String encoding, WritableByteChannel outputChannel) throws IOException, ClosedConnectionException {
			return getUnderlyingConnection().transferAvailableByDelimiter(delimiter, encoding, outputChannel);
		}
		...
		public int indexOf(String str, int maxLength) throws IOException, ClosedConnectionException, MaxReadSizeExceededException {
			return getUnderlyingConnection().indexOf(str, maxLength);
		}
		...
		public int indexOf(String str, String encoding, int maxLength) throws IOException, ClosedConnectionException, MaxReadSizeExceededException {
			return getUnderlyingConnection().indexOf(str, encoding, maxLength);
		}
