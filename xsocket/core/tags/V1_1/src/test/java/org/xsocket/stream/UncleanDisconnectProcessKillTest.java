...
public final class UncleanDisconnectProcessKillTest {
	...
	@Test public void testBlockingClientServerClose() throws Exception {
	...
	@Test public void testNonBlockingClientServerClose() throws Exception {
	...
	@Test public void testClientClose() throws Exception {
		...
		// start the client
		JavaProcess jp = new JavaProcess();
		jp.start("org.xsocket.stream.TestClient", "http://xsocket.sourceforge.net/test.jar", server.getLocalAddress().getHostName(), Integer.toString(server.getLocalPort()));
		QAUtil.sleep(500);
