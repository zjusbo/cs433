...
public class BlockingConnection implements IBlockingConnection {
	...
	private long waitForData(long remainingTime) throws ClosedChannelException, SocketTimeoutException {
		...
		if (isOpen()) {
			...
			if (remainingTime > 0) {
				...
				if (delegee instanceof NonBlockingConnection) {
					NonBlockingConnection nbc = (NonBlockingConnection) delegee;
	...
	// execute mode is hard-coded in Connection utils -> non-synchronized
