...
public final class NonBlockingConnectionPool implements IConnectionPool {
	...
	private static final int MIN_REMAINING_SEC_TO_IDLE_TIMEOUT = 3 * 1000;
	private static final int MIN_REMAINING_SEC_TO_CONNECTION_TIMEOUT = 3 * 1000;
	...
	private final class PooledConnectionHolder implements IInternalHandler {
		...
		boolean isValid(long currentTimeSec) {
			...
			if (connection.getRemainingSecToIdleTimeout() < MIN_REMAINING_SEC_TO_IDLE_TIMEOUT) {
				return false;
			}

			if (connection.getRemainingSecToConnectionTimeout() < MIN_REMAINING_SEC_TO_CONNECTION_TIMEOUT) {
				return false;
			}
	...
	private final class NonBlockingConnectionProxy implements INonBlockingConnection {
		...
		public void setConnectionTimeoutSec(int timeoutSec) {
			if (isProxyOpen) {
				pooledConnectionHolder.getConnection().setConnectionTimeoutSec(timeoutSec);
		...
		public int getConnectionTimeoutSec() {
			return pooledConnectionHolder.getConnection().getConnectionTimeoutSec();
		}

		public void setIdleTimeoutSec(int timeoutInSec) {
			if (isProxyOpen) {
				pooledConnectionHolder.getConnection().setIdleTimeoutSec(timeoutInSec);
		...
		public int getIdleTimeoutSec() {
			return pooledConnectionHolder.getConnection().getIdleTimeoutSec();
		}
		...
		public int getRemainingSecToIdleTimeout() {
			return pooledConnectionHolder.getConnection().getRemainingSecToIdleTimeout();
		}
		...
		public ByteBuffer[] readAvailableByteBuffer() throws IOException {
			return pooledConnectionHolder.getConnection().readAvailableByteBuffer();
		}
