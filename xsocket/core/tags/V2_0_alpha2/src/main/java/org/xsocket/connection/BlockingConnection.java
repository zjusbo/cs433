...
*    * @throws IOException If some other I/O error occurs
	...
	public int getRemainingSecToConnectionTimeout() {
		return delegee.getRemainingSecToConnectionTimeout();
	}
	...
	public int getRemainingSecToIdleTimeout() {
		return delegee.getRemainingSecToIdleTimeout();
	}
	...
	private final class ReadNotificationHandler implements IInternalHandler {
		...
		public boolean onData(INonBlockingConnection connection) throws IOException, BufferUnderflowException, MaxReadSizeExceededException {
			if (LOG.isLoggable(Level.FINE)) {
				LOG.fine("got data: wake up waiting threads (available read queue size=" + delegee.available() + ")");
			}
			onReadDataInserted();
			return true;
		}
		...
		public boolean onDisconnect(INonBlockingConnection connection) throws IOException {
			if (LOG.isLoggable(Level.FINE)) {
				LOG.fine("disconnected: wake up waiting threads");
			}
			onReadDataInserted();
			return true;
		}
		...
		public boolean onConnectionTimeout(INonBlockingConnection connection) throws IOException {
			if (LOG.isLoggable(Level.FINE)) {
				LOG.fine("connection timeout: wake up waiting threads");
			}
			onReadDataInserted();
		...
		public boolean onIdleTimeout(INonBlockingConnection connection) throws IOException {
			if (LOG.isLoggable(Level.FINE)) {
				LOG.fine("idle timeout: wake up waiting threads");
			}
			onReadDataInserted();
