...
public abstract class AbstractNonBlockingStream implements WritableByteChannel, Closeable, Cloneable {
	...
	public final ByteBuffer[] readAvailableByteBuffer() throws IOException {
		ByteBuffer[] buffers = readQueue.readAvailable();
		if (buffers == null) {
	...
	public final long write(ByteBuffer[] buffers) throws IOException, BufferOverflowException {
		if (isOpen()) {
			...
			if (buffers.length == 0) {
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("warning lenth of buffer array to send is 0");
				}
				return 0;
			}
	...
	public long transferFrom(ReadableByteChannel source) throws IOException, BufferOverflowException {
		return transferFrom(source, 8192);
	}
	...
	/**
	 * transfers data from a source channel to a target channel 
	 * 
	 * @param source     the source channel
	 * @param target     the target channel
	 * @param chunkSize  the transfer chunk size 
	 * @return the number of transfered bytes
