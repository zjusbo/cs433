...
public final class NonBlockingConnection extends AbstractNonBlockingStream implements INonBlockingConnection {
	...
	public void setHandler(IHandler hdl) throws IOException {	
		handlerAdapterRef.set(HandlerAdapter.newInstance(hdl));
	...
	private void onDisconnect() {
		...
		if (!disconnectOccured.getAndSet(true)) {
			...
			// call first onData
			try {
				handlerAdapterRef.get().onData(NonBlockingConnection.this, taskQueue, workerpool, true, false);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
					LOG.fine("[" + getId() + "] error occured by performing onData callback on " + handlerAdapterRef.get() + " " + ioe.toString());
				}
			...
			// then onDisconnect
			try {
				handlerAdapterRef.get().onDisconnect(NonBlockingConnection.this, taskQueue, workerpool, false);
			} catch (IOException ioe) {
				if (LOG.isLoggable(Level.FINE)) {
